 ...
 */
static inline size_t argo_ring_has_data_nowrap(const struct argo_ring_hnd *h)
{
...
int argo_core_init(void)
{
	...
	rc = bind_virq_to_irqhandler(argo_virq, 0, argo_interrupt, 0,
		"argo", NULL);
	if (rc < 0)
		return rc;
...
{
	unbind_from_irqhandler(argo_virq, NULL);
}
