 ...
 */
static int cargo_sanity(const struct cargo_args *args)
{
    assert(args != NULL);
 ...
 * Recving end.
 */
static int cargo_recvfrom_run(unsigned long port)
{
 ...
 * Sending side.
 */
static int cargo_sendto_run(unsigned int domid, unsigned long port)
{
    int s, rc;
#define AUTOBIND
#ifndef AUTOBIND
    ...
    while (1) {
        ...
        do {
            ...
            printd("%zuB sent to dom%u:%u.", rc, sa.svm_cid, sa.svm_port);
            offset += rc;
            msg_len -= rc;
        } while (msg_len != 0);
    }
...
static int cargo_run(const struct cargo_args *args)
{
    int rc;

    assert(args != NULL);

    rc = cargo_sanity(args);
    if (rc < 0)
        return -rc;

    if (args->recv)
        rc = cargo_recvfrom_run(args->local_port);
    else
        rc = cargo_sendto_run(args->domid, args->port);
...
{
    printi("Basic usage:");
    printi("	cargo domid port");
    printi("	cargo -r -p local_port");
    printi("Options:");
        ...
        switch (opt) {
            ...
            case 'p':
                ...
                if (rc || !is_valid_port(args.local_port)) {
                    printe("Invalid local port %lu.", args.local_port);
                    return -rc;
                }
                continue;
            case 'D':
